# CAPP!C_AI 논문 리포트 (2026-02-20)

> 수집 27 | 필터 25 | 폐기 7 | 평가 21 | 출력 8 | 기준 50점

검색 윈도우: 2026-02-18T23:30:00+00:00 ~ 2026-02-20T00:30:00+00:00 | 임베딩: en_synthetic | run_id: 4

---

## 검색 키워드

autonomous cinematography, camera control, sports filming, highlight detection, event detection, keyframe extraction, video enhancement, video correction, video stylization, image enhancement, image correction, image stylization, pose estimation, human pose, sports biomechanics, action recognition, activity recognition, motion analysis, sports analytics, tactical analysis, performance metrics, multi-object tracking, player tracking, object tracking, edge computing, embedded vision, edge ai, sports vision, real-time processing, video summarization

---

## 1위: A.R.I.S.: Automated Recycling Identification System for E-Waste Classification Using Deep Learning

- arXiv: http://arxiv.org/abs/2602.17642v1
- PDF: https://arxiv.org/pdf/2602.17642v1
- 발행일: 2026-02-19
- 카테고리: cs.LG
...
**활용 인사이트**
YOLOx 모델을 rk3588 엣지 디바이스에 적용해 선수 동작 인식 속도를 개선합니다. inference speed 30fps 이상 유지하며, 실시간으로 주요 장면(예: 골 장면)을 자동 추출해 편집 프로세스의 latency를 50ms 이하로 줄입니다.

## 2위: AudioChat: Unified Audio Storytelling, Editing, and Understanding with Transfusion Forcing

- arXiv: http://arxiv.org/abs/2602.17097v1
- PDF: https://arxiv.org/pdf/2602.17097v1
- 발행일: 2026-02-19
- 카테고리: cs.SD
...
**선정 근거**
오디오 기반 자동 스토리텔링 기술로, 비디오 하이라이트 생성과 유사한 개념이지만, 우리 프로젝트의 주된 비디오 분석과 직접적 연관성이 낮아 적용 우선순위가 비교적 낮습니다.

**활용 인사이트**
주요 적용 분야는 한계적이지만, 오디오 트랜스포머를 활용해 경기 중 관중 반응이나 코치 지시음을 분석해 보조 데이터로 사용 가능합니다. 다만 비디오 콘텐츠 생성에는 제한적으로 기여합니다.

## 3위: Secure Task Offloading and Resource Allocation Design for Multi-Layer Non-Terrestrial Networks

- arXiv: http://arxiv.org/abs/2602.17192v1
- PDF: https://arxiv.org/pdf/2602.17192v1
- 발행일: 2026-02-19
- 카테고리: eess.SP
...
**선정 근거**
엣지 컴퓨팅과 태스크 오프로딩 기술이 rk3588 기기에서 고성능 영상 분석을 위해 간접적으로 중요합니다. 리소스 제약 환경에서 효율적인 계산 분산이 핵심 과제이기 때문입니다.

**활용 인사이트**
계층적 오프로딩을 도입해 무거운 AI 처리(예: 자세 분석)를 클라우드로 전송합니다. 이를 통해 엣지 디바이스의 inference latency를 100ms 미만으로 유지하고, 실시간 성능을 보장합니다.

## 4위: A Hybrid Federated Learning Based Ensemble Approach for Lung Disease Diagnosis Leveraging Fusion of SWIN Transformer and CNN

- arXiv: http://arxiv.org/abs/2602.17566v1
...
**선정 근거**
CNN/Transformer 기술은 적용 가능하나, 의료 영상 분석에 특화되어 스포츠 영상의 동적 움직임 분석과는 도메인 차이가 큽니다. 직접적 연관성은 낮아 활용도가 제한적입니다.

**활용 인사이트**
SWIN Transformer 구조를 전이 학습해 스포츠 자세 인식 모델에 재활용할 수 있습니다. parameter count 100M 이하로 최적화해 rk3588에서 실시간 구동 가능하도록 적용합니다.

## 5위: Hierarchical Edge-Cloud Task Offloading in NTN for Remote Healthcare

- arXiv: http://arxiv.org/abs/2602.17209v1
- PDF: https://arxiv.org/pdf/2602.17209v1
- 발행일: 2026-02-19
- 카테고리: cs.NI
...
**선정 근거**
엣지-클라우드 오프로딩 기술이 간접적으로 유용합니다. 원격 의료 시나리오와 유사하게 스포츠 현장의 리소스 제약을 해결할 수 있어, 프로젝트의 실시간 데이터 처리에 기여합니다.

**활용 인사이트**
HAPS 모델을 참조해 로컬 엣지(rk3588)에서 기본 분석 후, 고급 작업(예: 전략 시뮬레이션)을 클라우드로 오프로드합니다. 이로써 end-to-end latency를 200ms 이내로 줄이고 시스템 효율성을 높입니다.

## 6위: Mechanistic Interpretability of Cognitive Complexity in LLMs via Linear Probing using Bloom's Taxonomy

- arXiv: http://arxiv.org/abs/2602.17229v1
...
**선정 근거**
LLM의 인지 수준 분석 기술이 스포츠 경기 해설 생성이나 전략 설명에 활용될 수 있음. 간접적 연관성으로 프로젝트의 AI 분석 기능 강화 가능.

**활용 인사이트**
경기 분석용 LLM 모델 개발 시 블룸 분류 적용해 다양한 인지 수준의 해설 자동 생성. 선수별 전략 설명 정확도 향상에 활용.

## 7위: NRGS-SLAM: Monocular Non-Rigid SLAM for Endoscopy via Deformation-Aware 3D Gaussian Splatting

- arXiv: http://arxiv.org/abs/2602.17182v1
...
**선정 근거**
비강성 SLAM 기술이 스포츠 영상의 움직임 추적에 적용 가능. 선수 동작 같은 비강성 요소 처리로 카메라 추적 정확도 향상.

**활용 인사이트**
3D 가우시안 기반 재구성 기술로 경기장 내 선수 동작 실시간 추적. 저지연(10ms 내)으로 하이라이트 장면 자동 편집에 적용.

## 8위: Contact-Anchored Proprioceptive Odometry for Quadruped Robots

- arXiv: http://arxiv.org/abs/2602.17393v1
- PDF: https://arxiv.org/pdf/2602.17393v1
- 발행일: 2026-02-19
- 카테고리: cs.RO, eess.SP
- 점수: final 66.4 (llm_adjusted:58 = base:50 + bonus:+8)
- 플래그: 실시간, 코드 공개
...
**선정 근거**
접지점 기반 오도메트리가 운동선수 발동작 추적에 유사 적용 가능. 드리프트 오류 감소로 경기 분석 정확도 향상.

**활용 인사이트**
발 접지 센서 데이터와 영상 융합해 선수 이동 경로 추적. 초당 60프레임 처리로 실시간 경기 전략 분석 시스템 구축.

---

## 다시 보기

### Pareto Optimal Benchmarking of AI Models on ARM Cortex Processors for Sustainable Embedded Systems (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17508v1
- 점수: final 100.0
...
### HybridPrompt: Bridging Generative Priors and Traditional Codecs for Mobile Streaming (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17120v1
- 점수: final 98.4
...
### Hybrid System Planning using a Mixed-Integer ADMM Heuristic and Hybrid Zonotopes (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17574v1
- 점수: final 92.0
...
### IRIS: Learning-Driven Task-Specific Cinema Robot Arm for Visuomotor Motion Control (1회째 추천)
...
-> 이 논문은 저비용 학습 기반 로봇 카메라 제어 방법을 제안합니다. 핵심은 인간 시연을 통한 시뮬모터 궤적 학습입니다. 스포츠 경기 자동 촬영을 위한 카메라 포지셔닝 시스템 구축에 핵심 기술이므로 중요합니다.

### EntropyPrune: Matrix Entropy Guided Visual Token Pruning for Multimodal Large Language Models (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17196v1
- 점수: final 88.0
...
-> 이 논문은 멀티모달 모델의 시각 토큰 프루닝 방법을 제안합니다. 핵심은 매트릭스 엔트로피 기반 중복 토큰 제거입니다. 스포츠 영상 분석 모델의 추론 속도 향상을 위해 FLOPs 68.2% 감소 기술이 중요합니다.

### Powering Up Zeroth-Order Training via Subspace Gradient Orthogonalization (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17155v1
- 점수: final 84.0
...
-> 에지 디바이스에서 메모리 효율적인 모델 학습이 필수적입니다. 제로스 오더 최적화 기술은 RK3588의 제한된 리소스에서 파인튜닝 속도와 정확도를 동시에 개선해 줍니다.

### RetouchIQ: MLLM Agents for Instruction-Based Image Retouching with Generalist Reward (1회째 추천)
...
-> 스포츠 이미지 자동 보정 핵심 기술입니다. MLLM 에이전트가 동작 포착 후 전문가 수준 보정을 실시간으로 수행해 제품 경쟁력 향상.

### WarpRec: Unifying Academic Rigor and Industrial Scale for Responsible, Reproducible, and Efficient Recommendation (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17442v1
- 점수: final 82.4
...
-> 플랫폼의 콘텐츠 추천 기능에 직접 활용 가능합니다. 산업 규모의 효율성을 보장하며 사용자 맞춤형 하이라이트 배포 체계 구축을 가능하게 합니다.

### Cross Pseudo Labeling For Weakly Supervised Video Anomaly Detection (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17077v1
- 점수: final 80.0
...
-> 경기 하이라이트 자동 탐지 핵심 솔루션입니다. 약한 감독 학습으로 골·파울 등 주요 장면 검출 정확도를 높여 편집 효율성 극대화.

### Selective Training for Large Vision Language Models via Visual Information Gain (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17186v1
- 점수: final 80.0
...
-> 시각적 근거 강화 기술로 운동 장면 분석 정확도 향상에 필수적. 언어 편향 감소로 색상, 공간 관계 등 객관적 분석 가능.

### AdvSynGNN: Structure-Adaptive Graph Neural Nets via Adversarial Synthesis and Self-Corrective Propagation (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17071v1
- 점수: final 80.0
...
-> 그래프 신경망 강화로 선수 간 상호작용 분석 가능. 노이즈 내성 높아 실외 경기 변수(광선/장애물)에서도 포즈·전략 예측 안정화.

### Catastrophic Forgetting Resilient One-Shot Incremental Federated Learning (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17625v1
- 점수: final 80.0
...
-> 에지 디바이스 증분 데이터 처리 최적화. 단일 통신으로 프라이버시 보장하며 하이라이트 영상 지속 학습 가능.

### FLoRG: Federated Fine-tuning with Low-rank Gram Matrices and Procrustes Alignment (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17095v1
- 점수: final 80.0
...
-> 에지 기기 협업 미세 조정 효율성 증대. 통신 오버헤드 2041배 감소로 다수 디바이스 실시간 협업 분석 가능.

### GraphThinker: Reinforcing Video Reasoning with Event Graph Thinking (1회째 추천)

- arXiv: http://arxiv.org/abs/2602.17555v1
- 점수: final 80.0
...
-> 경기 전략 분석 및 자세 교정에 필수적인 비디오 추론 기술입니다. 이벤트 그래프를 통해 스포츠 동작 간 인과관계 정밀 분석이 가능합니다.
